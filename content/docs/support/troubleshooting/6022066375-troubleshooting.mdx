...
**My paywall has unexpected presentation behavior**
---------------------------------------------------
...
**In iOS, a transaction appeared active when it wasn't**
--------------------------------------------------------

This can sometimes be related to a target membership issue. To resolve it, try removing all transactions or re-adding the [StoreKit testing file](https://superwall.com/docs/testing-purchases). Here's how:

1\. Open Xcode and navigate to **Debug → StoreKit → Manage Transactions**.

2\. Delete any existing transactions.

3\. If the issue persists, remove your StoreKit configuration file from the project.

4\. Build the project, then re-add the StoreKit file.
...
If your products haven't loaded you'll get an error message printed to the console. A common symptom of products not loading is payment buttons on your paywall becoming unresponsive when tapped. Run through the following list to solve this:
