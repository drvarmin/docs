...
const controller = {
  onPurchase: async (params) => {
    try {
      ...
      throw error // Re-throw to indicate failure to Superwall
    }
  },
  onPurchaseRestore: async () => {
    try {
...
- For RevenueCat integration, see the [Using RevenueCat](/docs/expo/guides/using-revenuecat) guide for complete examples